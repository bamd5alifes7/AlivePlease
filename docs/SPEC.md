...
- `is_first_launch` 為 `true` 時不建立打卡提醒，並取消既有打卡提醒工作。
- 完成或略過 Onboarding 後才開始排程打卡提醒。
...
- 使用者可在設定頁選填生日月日，資料僅儲存在裝置本機。
- 生日當天：
  - 生日邀請 Banner 顯示在首頁內容最上方，不要求事先打卡。
  - 當天第一次實際進入首頁時，顯示一次生日邀請對話框。首頁在 Pager 中預先建立時不視為已進入。
  - 關閉對話框不會隱藏 Banner，使用者可於當天不限次數重玩。
- 許願體驗包含三根蠟燭與三個願望，使用者逐根向左滑動熄滅蠟燭，並取得隨機願望類型祝福。
- 非生日當天不顯示邀請或彩蛋入口。非閏年的 2 月 29 日生日於 2 月 28 日觸發。
...
- 生日必須月日同時留空或同時填寫，並符合實際日曆日期，含 2 月 29 日。
...
- 首次引導未完成時不排程；Worker 若在此狀態被觸發，會取消打卡提醒並結束。
- 尚未打卡時，首則提醒的初始延遲為完整 `notify_interval`，不會在首次開啟 App 時立即發送。
- 已有打卡紀錄時，排程會依 `last_check_in_time + notify_interval` 計算初始延遲。
...
| `birthday_month` | 選填生日月份，`1..12` | `0` |
| `birthday_day` | 選填生日日期，`1..31` | `0` |
| `birthday_prompt_shown_date` | 最近顯示首次生日邀請的日期，`yyyy-MM-dd` | 空 |
...
- 首次引導未完成時不排程打卡提醒，尚未打卡時的首則提醒延遲完整設定週期。
- 打卡後打卡提醒重新排程，未超過提醒週期時不發提醒。
- 生日日期比對、非閏年 2 月 29 日觸發與每日首次邀請規則。
