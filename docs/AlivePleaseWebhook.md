...
每封正式親友通知都會帶有 `requestId`。新版 GAS 會記住已成功寄出的 `requestId`，即使 App 因網路回應遺失而重送相同請求，也不會再次寄出同一封信。

如果你已經架設過自訂 GAS，請重新複製最新版 [`AlivePleaseWebhook.gs`](AlivePleaseWebhook.gs)，並依照下方「更新已部署的程式」建立新版本，才會啟用 GAS 端的重複寄信防護。
...
Alive Please 會逐位寄送，因此部分成功、部分失敗是可能的。App 重試時只會再次寄送尚未成功的通知對象，不會重寄給已成功的人。請查看 App 執行紀錄與 Apps Script `Executions`，確認失敗收件人的 Email 與寄信配額。

### 為什麼同一段逾期期間只收到一封信？

每次打卡會開始新的通知週期。某位通知對象在該週期成功收到親友通知後，開啟 App、重新啟動手機或重新排程都不會再次寄信。使用者再次打卡後，才會開始下一個通知週期。
...
```json
{
  ...
  "body": "message text",
  "requestId": "1718150400000:family@example.com"
}
```
...
- `requestId`：正式親友通知的唯一識別碼。相同 `requestId` 成功寄出後再次送達時，GAS 會回報成功但不會重複寄信。測試信可省略。
...
- `mail_duplicate_ignored`：收到已成功處理過的 `requestId`，因此未重複寄信。
